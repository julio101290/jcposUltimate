    ...
    public function mostrarTablaVentas($columnas, $filtros, $busqueda) {
        ...
        $datosJson = '
      {
        ...
        for ($i = 0; $i < count($ventas); $i++) {
            ...
            $botones .= "<button type='button' class='btn btn-info  btnMasInfo' data-toggle='modal' data-target='#modalMasInfo' data-dismiss='modal' idVenta='" . $ventas[$i]["id"] . "' idCodigo='" . $ventas[$i]["codigo"] . "' idCliente='" . $ventas[$i]["id_cliente"] . "' puntoVenta='" . $ventas[$i]["puntoVenta"] . "'  required data-toggle='tooltip' data-placement='top' title='Mas Info'><i class='fa fa-info'></i> </button>";

            $botones .= "<button class='btn btn-info btnImprimirTicket' required data-toggle='tooltip' data-placement='top' title='Imprimir ticket' codigoVenta='" . $ventas[$i]["UUID"] . "'><i class='fa fa-ticket'></i></button>";
